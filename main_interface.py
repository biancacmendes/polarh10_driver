...
# Imports originais da sua aplicação
...
class CoreAsyncWorker(QThread):
    """
    Thread dedicada para rodar a rotina original da main.py.
    Garante que o Bluetooth e o Servidor Uvicorn/FastAPI rodem sem travar a interface gráfica.
    """
    status_updated = pyqtSignal(str, int)  # Mensagem, porcentagem da barra
    data_emitted = pyqtSignal(dict)
    hub_command = pyqtSignal(str)          # Sinal que avisa a GUI para disparar START/STOP
    ...
    async def _lifecycle(self):
        try:
            ...
            # Dispara o gateway em background para não travar o fluxo da QThread
            ...
            # Força uma tarefa paralela para monitorar os estados internos do gateway de rede
            asyncio.create_task(self._check_hub_status())

            # Loop contínuo de consumo da fila nativa
    ...
    async def _check_hub_status(self):
        """
        Varre o estado do barramento do gateway para capturar as mudanças de gravação
        geradas pelos comandos remotos e repassar os sinais para a interface Qt.
        """
        last_state = False
        while not self.is_cancelled:
            await asyncio.sleep(0.1)
            if self.gateway:
                # Verifica se a sua aplicação ativa alguma flag interna ao printar RECORDING STARTED
                # Caso seu gateway use uma propriedade como self.gateway.is_recording ou similar:
                current_state = getattr(self.gateway, "is_recording", None)
                
                # Fallback alternativo: se o próprio polar client gerencia o buffer ativo do experimento
                if current_state is None and self.polar:
                    current_state = getattr(self.polar, "is_recording", False)
                
                # Se houver mudança de estado detectada no barramento assíncrono, emite para a janela
    ...
    def cancel_operation(self):
        """Sinaliza a interrupção precoce dos objetos assíncronos em execução."""
        self.is_cancelled = True
        if self.loop and self.loop.is_running():
            for task in asyncio.all_tasks(self.loop):
                task.cancel()
            self.loop.create_task(self._clean_sockets())

    async def _clean_sockets(self):
        try:
            if self.gateway: await self.gateway.stop()
            if self.polar: await self.polar.disconnect()
        except:
            pass
        finally:
            self.loop.stop()
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setStyleSheet("""
            ...
            QLabel { color: #2d3748; }
            QLineEdit, QComboBox { background-color: #ffffff; border: 1px solid #cbd5e1; border-radius: 4px; padding: 6px; }
            QGroupBox { font-weight: bold; border: 1px solid #e2e8f0; border-radius: 6px; margin-top: 12px; padding-top: 16px; background-color: #ffffff; }
            QPushButton { border-radius: 4px; font-weight: bold; padding: 8px 16px; }
        """)
        ...
        # Construção das telas estruturadas
        ...
        # Dispara o motor em segundo plano
    ...
    def create_page_loading(self):
        ...
        self.progress_bar.setStyleSheet("""
            QProgressBar { border: 1px solid #cbd5e1; border-radius: 4px; text-align: center; background-color: #ffffff; }
            QProgressBar::chunk { background-color: #2563eb; width: 10px; }
        """)
        ...
        self.btn_cancel_search.setStyleSheet("""
            QPushButton { background-color: #ef4444; color: white; padding: 10px; margin-top: 10px; }
    ...
    def handle_post_cancellation(self):
        ...
        QMessageBox.information(self, "Cancelado", "A busca pelo dispositivo e a inicialização dos gateways foram interrompidas.")

    # --- TELA 1: CADASTRO DO PACIENTE ---
    def create_page_form(self):
        ...
        btn_next = QPushButton("Avançar para Etapa de 10 Minutos")
    ...
    # --- TELA 2: ETAPA 1 (GRAVAÇÃO OPCIONAL 10 MINUTOS) ---
    def create_page_stage_fixed(self):
        page = QWidget()
        layout = QHBoxLayout()

        left_panel = QVBoxLayout()
        left_panel.addWidget(QLabel("<b>ETAPA 1: Gravação Estática</b>"))
        
        self.lbl_timer_fixed = QLabel("10:00")
        self.lbl_timer_fixed.setStyleSheet("font-size: 32px; font-weight: bold; color: #1e3a8a;")
        left_panel.addWidget(self.lbl_timer_fixed)

        self.btn_start_fixed = QPushButton("Iniciar 10 Minutos")
        ...
        self.ecg_curve_fixed = self.ecg_widget_fixed.plot(pen=pg.mkPen("#ef4444", width=1.5))
    ...
    def start_fixed_recording(self):
        ...
        self.prepare_storage_paths("Etapa_Fixa")
    ...
    def handle_fixed_countdown(self):
        if self.timer_counter > 0:
            ...
            QMessageBox.information(self, "Concluído", "Gravação estática finalizada!")
    ...
    # --- TELA 3: ETAPA 2 (SINCRONIZADA VIA HUB) ---
    def create_page_stage_hub(self):
        ...
        self.lbl_hub_status.setStyleSheet("font-size: 12px; font-weight: bold; color: #475569; padding: 8px; background-color: #e2e8f0; border-radius: 4px;")
        left_panel.addWidget(self.lbl_hub_status)

        self.lbl_recording_indicator = QLabel("● GRAVANDO NO DISCO")
        self.lbl_recording_indicator.setStyleSheet("color: #ef4444; font-weight: bold; font-size: 14px;")
        ...
        self.ecg_widget_hub = pg.PlotWidget(title="ECG Bruto em Tempo Real (Monitoramento Remoto)")
        self.ecg_widget_hub.setBackground("#ffffff")
        self.ecg_curve_hub = self.ecg_widget_hub.plot(pen=pg.mkPen("#ef4444", width=1.5))
    ...
    def handle_hub_remote_trigger(self, command):
        if command == "START" and not self.is_recording_hub:
            ...
            self.lbl_hub_status.setStyleSheet("font-size: 12px; font-weight: bold; color: white; padding: 8px; background-color: #b91c1c; border-radius: 4px;")
            ...
            self.lbl_hub_status.setStyleSheet("font-size: 12px; font-weight: bold; color: white; padding: 8px; background-color: #15803d; border-radius: 4px;")
            self.lbl_recording_indicator.setVisible(False)
            self.finalize_npy_save()

    # --- SALVAMENTO EM FORMATO .NPY ---
    ...
    def process_live_stream(self, data):
        ...
        if metrics and (self.is_recording_fixed or self.is_recording_hub):
            ...
            if rr_val is not None:
                self.save_hrv_buffer.append([ts, float(rr_val), metrics.get("hr", 0)])

    def finalize_npy_save(self):
        if self.save_ecg_buffer:
            ecg_array = np.array(self.save_ecg_buffer, dtype=np.float64)
            np.save(os.path.join(self.participant_folder, f"{self.file_prefix}_ecg.npy"), ecg_array)
            
        if self.save_hrv_buffer:
            hrv_array = np.array(self.save_hrv_buffer, dtype=np.float64)
            np.save(os.path.join(self.participant_folder, f"{self.file_prefix}_hrv.npy"), hrv_array)
